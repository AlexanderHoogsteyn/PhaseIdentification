...
results = []
for salient_component in [0,1,2,4]:
    class_results = []
    for feeder_id in included_feeders:
        ...
        for rep in np.arange(reps):
            ...
            phase_identification.load_correlation_xu_fixed(salient_components=salient_component,length=length,nb_salient_components=nb_salient_components)
...
rects3 = ax.bar(x - width*3/2, results[0], width, label='Raw')
rects4 = ax.bar(x - width/2, results[1], width, label='Delta m-1')
rects5 = ax.bar(x + width/2, results[2], width, label='Delta m-2')
rects6 = ax.bar(x + width*3/2, results[3], width, label='Delta m-4')
